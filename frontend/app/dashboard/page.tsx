...
type StockThesisFinding = { ticker: string; sector: string; growth_drivers: string[]; decline_risks: string[]; pros: string[]; cons: string[]; narrative?: string };
type StockThesis = { applicable: boolean; reason_if_not_applicable?: string; findings: StockThesisFinding[] };
type SectorThesisFinding = { sector: string; growth_drivers: string[]; headwinds: string[]; policy_geopolitical_factors: string[]; pros: string[]; cons: string[]; narrative: string };
type SectorThesis = { findings: SectorThesisFinding[] };
type RiskReport = { summary: { portfolio_risk_level: string; diversification_verdict: string; sarvam_insight?: string | null }; executive_report?: { sector_commentary: string; asset_commentary: string; risk_commentary: string; stock_thesis_commentary?: string; sector_thesis_commentary?: string; recommendations: string[] }; stock_level_risk_profiles: Array<{ ticker: string; sector: string; overall_risk_rating: string }>; stock_thesis?: StockThesis; sector_thesis?: SectorThesis; portfolio_diversification_analysis: { sector_allocation: Array<{ sector: string; allocation_pct: number }>; concentration_flags: Array<{ sector: string; allocation_pct: number }>; under_exposed_or_missing_sectors: string[] } };
...
export default function DashboardPage() {
  ...
  async function loadDashboard() {
    ...
    if (portfolioResponse.ok) {
      ...
      // from the AI analysis, which now only runs when explicitly requested.
  ...
  async function runAiAnalysis() {
    ...
    const headers = { Authorization: `Bearer ${auth}` };
    try {
      const riskResponse = await fetch(`${API_URL}/api/analysis/risk-profile`, { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body: JSON.stringify({ holdings: holdings.map((holding, index) => ({ ticker: holding.symbol, quantity: holding.quantity, buy_price: holding.buy_price, current_price: analysis?.positions?.[index]?.quote?.price ?? holding.buy_price })), include_llm_insight: true }) });
      if (riskResponse.ok) {
        setRiskReport(await riskResponse.json());
      } else if (riskResponse.status === 503) {
        setRiskReport(null);
        setAiError("AI analysis is temporarily unavailable. Please retry.");
      } else {
        setRiskReport(null);
        setAiError("Could not generate AI analysis. Please try again.");
      }
    } catch {
      setAiError("Could not reach the analysis service. Please try again.");
    }
    setAiBusy(false);
  }
  ...
  useEffect(() => {
    ...
    const clearAnalysis = () => { setAnalysis(null); setRiskReport(null); setAiError(""); window.sessionStorage.removeItem("portfolio-analysis"); };
  ...
  async function remove(id: string) {
    ...
    setAnalysis(null); setRiskReport(null); setAiError("");
  ...
  async function clearPortfolio() {
    ...
    if (responses.every((response) => response.ok)) { setHoldings([]); setHistories({}); setOpenCharts({}); setAnalysis(null); setRiskReport(null); setAiError(""); setIsAddPanelOpen(true); window.sessionStorage.removeItem("portfolio-analysis"); setMessage("All holdings cleared."); }
