...
export default function DashboardPage() {
  ...
  return <div className={styles.wrapper}>
    ...
    {holdings.length === 0 ? <div className={styles.empty}>Add a holding manually or import the spreadsheet to begin.</div> : <div className={styles.holdingList}>{analysis && <div className={styles.metrics}><div><small>Total invested</small><strong>{money(analysis.total_invested)}</strong></div><div><small>Current value</small><strong>{money(analysis.total_current)}</strong><em>{analysis.price_coverage}/{analysis.positions.length} prices retrieved</em></div><div><small>Portfolio change</small><strong className={analysis.total_current >= analysis.total_invested ? styles.goodText : styles.badText}>{money(analysis.total_current - analysis.total_invested)}</strong></div></div>}{holdings.map((holding) => { const item = analysis?.positions.find((position) => position.holding.id === holding.id); return <article className={styles.stockCard} key={holding.id}><div className={styles.stockTop}><div><span className={styles.stockSymbol}>{holding.symbol}</span><span className={styles.stockName}>{holding.company_name}</span></div><button className={styles.removeButton} onClick={() => remove(holding.id)}>Remove</button></div><div className={styles.holdingMeta}><span>{holding.quantity ?? "—"} shares</span><span>Avg {money(holding.buy_price)}</span>{item && <><span>Market {money(item.quote?.price)}</span><span>Invested {money(item.invested_amount)}</span><span>Current {money(item.current_amount)}</span></>}{item?.error && <span className={styles.badText}>{item.error}</span>}</div></article>; })}</div>}
  </div>;
}
